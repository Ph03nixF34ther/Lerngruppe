# Übersicht Elektrotechnik

Im folgenden stehen alle wichtigen grundlegenden Formel für die Elektroctechnick, sowie die zusammensetzung ihrer Einheiten.
# Stoff
- Physikalische Grundlagen und Umformung von Gleichungen
- Gleichstrom-/Gleichspannung
	- [[Elektrische Stromstärke]], [[Elektrische Stromdichte]], [[Elektrische Spannung]]
	- [[Elektrischer Widerstand|Widerstand]], [[Elektrischer Leitwert]]
	- [[Spezifischer Widerstand]], [[Spezifischer Leitwert]]; lineare und nichtlineare [[Elektrischer Widerstand|Widerstände]]
	- [[Arbeit]] und [[Leistung bei Gleichstrom]]
	- [[Bezugssinn und Pfeilsysteme]]
	- [[Kirchhoff'sche Gesetze]]
	- [[Spannungsquelle]]und [[Stromquellen]] mit Innenwiderstand/-leitwert
	- Berechnung von Gleichstromkreisen ([[Reihenschaltung|Reihenschaltungen]] / [[Parallelschaltung|Parallelschaltungen]]) 
	- [[Stern-Dreieck-Transformation]]
	- Netzwerkberechnungen für Netzwerke mit Spannungsquellen nach weiteren Verfahren ([[Maschenstromverfahren]], [[Knotenpotenzialverfahren]], [[Überlagerungsverfahren]])
	- [[Ersatzspannungsquelle|Ersatzspannungsquellen]] und [[Ersatzstromquelle|Ersatzstromquellen]]
	- [[Leistungsanpassung]]
	- [[Nichtlineare Stromkreise]] (Vorschau auf [[Diode]], nichtlineare Strom-/Spannungskennlinie)

- Elektrisches Feld
	- Feldstärke/Potenzial/Feldlinien
	- Elektrischer Fluss/Flussdichte
	- Berechnung elektrostat. Felder
	- Materie im el. Feld
	- Kapazität (am Beispiel Plattenkondensator, keine weiteren Geometrien)
	- Energie des elektrostat. Felds
	- Kräfte zwischen Ladungen (am Beispiel zweier Punktladungen)

- Magnetisches Feld
	- Feldstärke/Flussdichte/Permeabilität/Fluss, Begriff der Spule
	- Mag. Spannung
	- Durchflutungsgesetz
	- Magn. Kreis
	- Berechnung mag. Felder
	- Materie im mag. Feld
	- Kräfte im mag. Feld, bewegte Ladung (Hall-Effekt)
	- Induktion (bewegter Leiter / zeitl. veränderl. Feld / Selbst-/Gegeninduktion)

- Wechselstromtechnik
	- Wechselgrößen: allgem. Begriffe, dann Übergang zu sinusförm. Größen
	- Mittelwerte: Gleichrichtwert, Effektivwert, Crestfaktor)
	- Zeigerdarstellung (nur zum Aufzeigen der Möglichkeit der Phasenverschiebung zwischen Strom und Spannung)
	- Komplexe Rechnung (Grundlagen, i.d.R. sind die Studierenden dankbar dafür)
	- Grundschaltungen, dabei Einführung der Begriffe Blindwiderstand, Blindleitwert, Impedanz, Admittanz)
		- Kreis mit Widerstand
		- Kreis mit Spule
		- Kreis mit Kondensator
		- Kombination der Bauelemente, Umwandung von Serien-/Parallelschaltungen

	- Leistung im Wechselstromkreis
		- Wirkleistung
		- Blindleistung
		- Scheinleistung
	- Komplexe Berechnung von Wechselstromkreisen mit Kirchhoff’schen Gesetzen
	- Leistungsanpassung
	- Hoch-/Tiefpass
	- Schwingkreise
	- Magn. Gekoppelte Kreise (Trafo)

- Schaltvorgänge
	- R-L-Kreis
	- R-C-Kreis