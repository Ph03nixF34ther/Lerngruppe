...
\end{bmatrix}
$$
1. Schritt: Das Charakteristisch Polynom der Matrix $A$ bilden durch $\det(I\cdot x-A)$
$$
I\cdot x-A =
\begin{bmatrix}
x-1 & -2 & 0 \\
...
$$
2. Die Nullstellen dieses Charakteristischem Polynoms sind die Eigenwerte. Diese werden in die Matrix $D$ auf der Hauptdiagonalen geschrieben
$$
...
\end{bmatrix}
$$
3. Jetzt wird die Matrix $T$ gebildet. Sie setzt sich aus den Eigenvektoren der Matrix $A$ zusammen. 
   Für die Berechnung der Eigenvektoren siehe [[Eigenwerte und Eigenvektoren]].
$$
T=
\begin{bmatrix}
1 & -1 & 1 \\
...
\end{bmatrix}
$$
4. Zu Letzt die Inverse der Matrix $T$ Berechnen und $T\cdot D\cdot t^-1 = A$ notieren.
...
2. Mit den Eigenwerten aus der Matrix D die gewünschte Operation durchführen. zum Beispiel aus jedem Wert die Wurzel ziehen, oder jeden Wert ins Quadrat rechnen.
...
1. Die Eigenwerte der Matrix $A$ mit dem Charakteristischem Polynom bestimmen. Siehe [[Klausuraufgaben#1.Man zerlege folgende Matrix A in die Form A=T*D*T {-1}|1.Aufgabe]].
$$
A = 
\begin{bmatrix}
1 & 2 & 0 \\
...
$$
2. Diese Eigenwerte werden in die Transponierte Vandermonde Matrix $V$ eingesetzt. 
$$
V = 
\begin{bmatrix}
5^0 & 5^1 & 5^2 \\
...
\end{bmatrix}
$$
3. In einen Vektor $b$ wird das Exponential der einzelnen Eigenwerte berechnet
$$
b =
\begin{bmatrix}
e^5 \\
...
$$
2. Da die erst drei Orthonormalpolynome gefordert sind werden für die Funktionen jeweils ein Zähler von 0 bis 2 erstellt. sollten die ersten Vier gefordert sein würden diese bis 3 gehen.
$$
i = 0..2; j=0..2
$$
3. Nun wird die Gramersche Matrix $G_{i,j}$ gebildet. Dafür werden die einzelnen eintrage mit den Ergebnissen des Integrals gefüllt dabei wird $p(x)$ zu $x^i$ und $q(x)$ zu $x^j$.
$$
...
\end{bmatrix}
$$
4. Mit dieser Matrix wird die Matrix $R$ mit der Cholesky-Zerlegung erstellt. Siehe dafür [[Cholesky-Zerlegung]] 
$$
R = 
\begin{bmatrix}
1 & 0 & 0 \\
...
\end{bmatrix}
$$
5. Im letzten Schritt wird die Inverse von $R$ mit einem Vektor, der gefüllt ist mit den einzelnen Potenzen von x Multipliziert. Die Einzelnen werte des Ergebnisvektor sind die Lösungen.
$$
R^{-1} \cdot
\begin{bmatrix}
x^0 \\
...
\end{bmatrix}
$$
2. Das Gleichungssystem wird so aufgeteilt das eine rechteckige Matrix entsteht
$$
\begin{bmatrix}
3 & 5 \\
5 & 4
...
\end{bmatrix}
$$
7. $g(z)$ ergibt sich aus $v(x)^T\cdot v(x)$ 
$$
g(z)= 
...
\end{bmatrix}
$$
1. Aus den Gitterpunkten Lassen sich x und y werte entnehmen. Diese sind $x_{0}= 1, x_{1}=2, x_{2}=3$ und $y_{0}=2, y_{1}=4, y_{2}=6$. Mit diesen werten werden zwei Vandermonde Matrizen befüllt. $Vx$ und $Vy$ 
$$
Vx = 
\begin{bmatrix}
1 & 1 & 1 \\
...
\end{bmatrix}
$$
6. Nun wird der Vektor Gleich Null gesetzt. Der unter teil wird Null für 
$$
...
\end{bmatrix}
$$
	Daraus lässt sich schließen das die Stelle (2,4) für $p(x,y)$ kritisch ist, da wenn diese Zahlen eingesetzt werden der obere Werte ebenfalls Null wird.
7. Nun wird die Hesse-Matrix $H(x,y)$ mit allen möglichen Zweiten Ableitungskombinationen gebildet:
$$
H(x,y)= 
\begin{bmatrix}
...
\end{bmatrix}
$$
1. Zuerst nimmt man sich den Mittleren Teil, ohne die Inverse. Die Überlegung ist, dieser Teil mit einem Vektor $v$ Multipliziert soll den hinteren Vektor ergeben
$$
\begin{bmatrix}
z & -1 & -1 \\
0 & z & -1 \\
...
\end{bmatrix}
$$
2. Diese Gleichung wird mit der Chramerschen Formel gelöst. Der erste Vektor gibt an, dass nur die erste Spalte ersätzt werden muss.
$$
f(z)=\frac{
\det
\begin{bmatrix} 
...
$$
3. An der stelle $z = 1i$ ergibt sich der Wert:
$$
h(z)= \frac{1}{-1}=-1
$$
4. Alternativ kann die Inverse bestimmt werden und mit den beiden Vektoren Multipliziert werden. Daraus ergibt sich ebenfalls $\frac{1}{z^2}$
$$
\begin{bmatrix}
z & -1 & -1 \\
0 & z & -1 \\
...
5. Nun wir der Vektor $v(x)$ gebildet aus der Inversen von $R$ und den einzelnen Gewichten von x
$$
...
$$
6. Geschwindigkeit:
Die Geschwindigkeit $v(t)$ wird aus dem Betrag der Matrix $A$ Multipliziert mit $x(t)$, beziehungsweise mit b gebildet.
$$
v(t)= |A\cdot x(t)|=|A\cdot b|
$$
7. Abstand zum Koordinatenursprung:
Der Abstand $s$ ergibt sich aus dem Betrag von $x(t)$ beziehungsweise der Betrag von $b$:
$$
s= |x(t)|=|b|
$$
